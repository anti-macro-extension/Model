...
(() => {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    try {
      // ✅ 사용자 차단 처리
      if (msg?.kind === "BLOCK_USER") {
        handleUserBlock(msg.payload, sender);
...
// ✅ 사용자 차단 함수
function handleUserBlock(payload, sender) {
  if (!sender?.tab?.id) {
    console.error("❌ 차단 실패: 탭 ID 없음");
    return;
  }
  ...
  const fullUrl = `${blockUrl}?${params.toString()}`;
  
  console.error(`🚨 매크로 확정 차단: 탭 ${tabId}을(를) blocked.html로 리디렉션`);
  
  // 탭을 차단 페이지로 이동
  chrome.tabs.update(tabId, { url: fullUrl }, () => {
    if (chrome.runtime.lastError) {
      console.error("❌ 리디렉션 실패:", chrome.runtime.lastError.message);
    } else {
      console.log("✅ 차단 페이지로 이동 완료");
      
      // 뱃지 업데이트
      chrome.action.setBadgeText({ text: "🚫", tabId: tabId });
      chrome.action.setBadgeBackgroundColor({ color: "#dc2626", tabId: tabId });
      chrome.action.setTitle({ 
        title: `매크로 사용자 차단됨 - ${payload.domain}`, 
        tabId: tabId 
      });
      
      // 알림 표시
      showBlockNotification(payload);
    }
  });
  
  // 통계 업데이트
  detectionStats.totalBlocks = (detectionStats.totalBlocks || 0) + 1;
}

function showBlockNotification(payload) {
  const confidencePercent = Math.round(payload.confidence * 100);
  
  chrome.notifications.create({
    type: "basic",
    iconUrl: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><circle cx="32" cy="32" r="30" fill="%23dc2626"/><text x="32" y="42" text-anchor="middle" fill="white" font-size="32">🚫</text></svg>',
    title: "🚨 매크로 사용자 차단",
    message: `도메인: ${payload.domain}\n신뢰도: ${confidencePercent}%\n유형: ${payload.type === 'mouse' ? '마우스' : '키보드'}\n\n페이지가 차단되었습니다.`,
    priority: 2
  });
}
